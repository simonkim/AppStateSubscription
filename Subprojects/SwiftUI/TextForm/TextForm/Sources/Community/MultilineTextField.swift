...
/// Credit: https://medium.com/better-programming/swiftui-multiline-content-fit-textfield-3abbdcedaabc
    ...
    var placeholderView: some View {
        Group {
            if shouldShowPlaceholder {
                Text(placeholder).foregroundColor(Color(.lightText))
                    .padding(.leading, 4)
                    .padding(.top, 8)
            }
        }
...
//        }
    ...
    final class Coordinator: NSObject, UITextViewDelegate {
        ...
        func textView(_ textView: UITextView, shouldChangeTextIn range: NSRange, replacementText text: String) -> Bool {
            if let count = (textView.text as NSString?)?.replacingCharacters(in: range, with: text).numberOfLines(),
